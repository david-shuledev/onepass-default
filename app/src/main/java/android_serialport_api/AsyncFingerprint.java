...
public class AsyncFingerprint extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case FP_GetImage:
                    ...
                    AsyncFingerprint.this.obtainMessage(FP_GetImage, valueGetImage, -1).sendToTarget();
                    break;
                case FP_UpImage:
    ...
    private synchronized int PSGetImage() {
        ...
        sendCommand(command);
        int length = SerialPortManager.getInstance().read(buffer, 20);
    ...
    private synchronized byte[] PSEnroll() {
        byte[] command = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte)
				...
				0x14};
        sendCommand(command);
        int length = SerialPortManager.getInstance().read(buffer, 200);
