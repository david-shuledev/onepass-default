...
public class Fingerprint {
    ...
    private boolean bIsUpImage = false;
    ...
    private void SendMessage(int msg, byte[] data) {
        if (phandler != null) phandler.obtainMessage(msg, data).sendToTarget();
    }
...
/*			byte[] st=new byte[14];
    ...
    public byte[] NetCommandProcessex(byte[] revdata) {
        switch ((byte) revdata[0]) {
            case ZCMD_TESTLINK: {
                byte[] buffer = {(byte) 0xef, 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff,
						(byte) 0xff, 0x08, 0x00, 0x06, 0x02, 0x01, 0x02, 0x02, 0x00, 0x15};
            ...
            case ZCMD_OPENDOOR: {
                byte[] buffer = {(byte) 0xef, 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff,
						(byte) 0xff, 0x07, 0x00, 0x03, 0x00, 0x00, 0x0a};
                vFingerprint.FP_RS422ex(buffer);
                return null;
            }
            case ZCMD_CLOSEALARM: {
            ...
            case ZCMD_DNUSERINFO: {    //�����û�
                switch ((byte) revdata[CMDPARPOS]) {
                    case STARTPKG: {    //�û���Ϣ
                        int len = getShort(revdata[6], revdata[5]);
                        System.arraycopy(revdata, CMDDATAPOS, TempVals.getInstance().tempInfo, 0,
								len);
                        TempVals.getInstance().fpCount = 0;
                    }
                    break;
                    case ENDPKG:    //����
                        UsersList.getInstance().AppendUser(TempVals.getInstance().tempInfo,
								TempVals.getInstance().tempFP);
                        break;
                    case NEXTPKG:    //ģ��
                        int len = getShort(revdata[6], revdata[5]);
                        System.arraycopy(revdata, CMDDATAPOS, TempVals.getInstance().tempFP,
								TempVals.getInstance().fpCount * 512, len);
    ...
    public void RS485(byte[] data) {
       // vFingerprint.FP_RS485();
        vFingerprint.FP_RS422(data);
    }
}
